...
class Library {
    ...
    Load(book) {
        if (this.books.length == 0) {
            ...
            this.seenBooks.forEach(function (v, i) {
                ...
                var likes = "";
                likes += obj.seenBooks[i].title + " - " + "<b>" + obj.seenBooks[i].opinion + "</b>" + "<br>" + "<br>";
                $(".end p").append(likes)
            ...
            $(".books h1").text(book.title);
            $(".books p").text(book.description);
            $(".books img").attr("src", book.img);
            $(".books a").attr("href", book.link);
            $(".books a").text("more info");
        }
    ...
    morebooks(n){
        ...
        $(".end").css("display", "none");
        $.ajax({
            url: "https://www.googleapis.com/books/v1/volumes?q=" + $("#search").val() + "&maxResults=10&startIndex=" + n,
